...
class TestOrderListAPIView(APITestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(**test_products[0])
...
class TestOrderDetailAPIView(APITestCase):
    ...
    def test_patch_order_valid_shipped(self):
    ...
    def test_patch_order_invalid(self):
    ...
    def test_patch_order_access_denied(self):
        other_user = User.objects.create_user(**test_other_user_register)
        data = {'status': 'shipped'}
        request = self.factory.patch(
            reverse('orders:detail', kwargs={'pk': self.order.pk}),
